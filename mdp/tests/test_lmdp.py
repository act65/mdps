...
def test_mdp_embedding_simple():
    ...
    # Assertions for test
    assert np.isclose(r, r_approx, atol=1e-2).all()


def test_mdp_embedding_random():
    """
    ...
    # r(s, a) = q(s) - KL(P(. | s, a) || p(. | s))
    ce = np.zeros((n_states, n_actions))
    ...
    assert np.isclose(r, r_approx, atol=1e-3).all()
...
def test_decoder_rnd():
    ...
    # Generate a realizable u
    pi_true = utils.softmax(rnd.standard_normal((n_states, n_actions)))
    u = np.einsum('ijk,jk->ij', P, pi_true)
...
if __name__ == "__main__":
    test_mdp_embedding_simple()
    test_lmdp_solver_simple()
